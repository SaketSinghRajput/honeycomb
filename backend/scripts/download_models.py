...
# The backend folder is one level up from this script (parents[1])
...
def download_tts_model() -> bool:
    ...
    try:
        ...
        # Set TTS cache directory before initializing
        os.environ["TTS_HOME"] = str(tts_dir)
        
        start_time = time.time()
        
        # Initialize TTS (automatically downloads model to TTS_HOME)
...
def check_disk_space() -> bool:
    ...
    try:
        ...
        if free_gb < 10:
            logger.error(f"✗ Insufficient disk space: {free_gb:.2f}GB free (10GB required)")
            return False
